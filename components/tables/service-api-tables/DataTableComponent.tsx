...
interface DataTableComponentProps {
    data: DataRow[];
    onDetailClick: (apiDetails: ApiDetails) => void;
}
...
const DataTableComponent: React.FC<DataTableComponentProps> = ({ data, onDetailClick }) => {
    const columns: TableColumn<DataRow>[] = [
        ...
        },
        {
            ...
            sortable: true,
            minWidth: '150px',
            cell: row => <div className="text-gray-800">{row.platform || 'N/A'}</div>,
        },
        {
            name: 'Detail',
            cell: (row) => (
                <button 
                    onClick={() => onDetailClick(row.api)} // Menggunakan row.api yang merupakan ApiDetails
